...
class FileExplorer {
    ...
    async load_root() {
        ...
        for (let directory of response) {
            if (!("Directory" in directory.file_type))
                continue;
            this.cache.push(new CachedDirectory(directory));
        ...
        for (let file of response) {
            if (!("File" in file.file_type))
                continue;
            this.cache.push(new CachedFile(file));
    ...
    on_undo_button_click() {
        let file_explorer = document["file_explorer"];
        if (file_explorer.path_history_selector > 0)
            --file_explorer.path_history_selector;
    ...
    on_redo_button_click() {
        let file_explorer = document["file_explorer"];
        if (file_explorer.path_history_selector < file_explorer.path_history.length)
            ++file_explorer.path_history_selector;
        file_explorer.path = file_explorer.path_history[file_explorer.path_history_selector];
        file_explorer.update_navigation_buttons();
        file_explorer.load_file_viewer_view()

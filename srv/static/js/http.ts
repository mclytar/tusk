...
export class HTTP<T> {
    ...
    static TRACE(url: string) {
        return new HTTP(Method.TRACE, url);
    }

    Accept(mime_type: string): HTTP<T> {
    ...
    body_form(data: FormData): HTTP<T> {
        this.body = data;
        return this;
    }

    async send(): Promise<HTTPResponse<T>> {
        let response = new Promise<HTTPResponse<T>>((resolve) => {
